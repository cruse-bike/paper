...
pair for additional trip purposes. The classification of trip purposes
used in the CRUSE tool was guided mainly by the trip purpose
...
The main result of the CRUSE project is a new and open access web
application and dataset on current and potential future cycling levels
in Ireland, with evidence provided at county and route segment levels.
Like the PCT on which CRUSE builds, a key feature of the results is that
they are systematic and available for all counties, and most routes on
which cycling is permitted, in Ireland.

\hypertarget{sec-national-results}{%
\subsection{National results}\label{sec-national-results}}

The CRUSE tool provides estimates of cycling potential at the national
level, as shown in \textbf{?@fig-national-results}.
...
\hypertarget{local-results}{%
\subsection{Local results}\label{local-results}}

To illustrate the results in urban areas, we took the top 4 cities in
Ireland by population: Dublin, Cork, Limerick and Galway. The results
for these cities are presented in Figure~\ref{fig-city-results}.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Results for one city are shown below}
\NormalTok{city\_name }\OtherTok{=} \StringTok{"Dublin"}
\NormalTok{city\_buffer }\OtherTok{=}\NormalTok{ zonebuilder}\SpecialCharTok{::}\FunctionTok{zb\_zone}\NormalTok{(city\_name)}
\NormalTok{city\_buffer\_5km }\OtherTok{=}\NormalTok{ city\_buffer }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{filter}\NormalTok{(circle\_id }\SpecialCharTok{\textless{}=} \DecValTok{2}\NormalTok{)}
\CommentTok{\# qtm(city\_buffer\_5km)}
\NormalTok{city\_buffer\_union }\OtherTok{=}\NormalTok{ sf}\SpecialCharTok{::}\FunctionTok{st\_union}\NormalTok{(city\_buffer\_5km)}
\NormalTok{rnet\_fastest }\OtherTok{=}\NormalTok{ sf}\SpecialCharTok{::}\FunctionTok{read\_sf}\NormalTok{(}\StringTok{"./../testout/dublin/rnet\_combined\_fastest.gpkg"}\NormalTok{)}
\NormalTok{rnet\_fastest\_city }\OtherTok{=}\NormalTok{ sf}\SpecialCharTok{::}\FunctionTok{st\_intersection}\NormalTok{(rnet\_fastest, city\_buffer\_union)}\CommentTok{\#}
\CommentTok{\# nrow(rnet\_fastest\_city)}
\CommentTok{\# table(rnet\_fastest\_city$\textasciigrave{}Bicycle (Go Dutch)\textasciigrave{})}
\NormalTok{rnet\_fastest\_city\_top }\OtherTok{=}\NormalTok{ rnet\_fastest\_city }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{filter}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{Bicycle (Go Dutch)}\StringTok{\textasciigrave{}} \SpecialCharTok{!=} \StringTok{"1{-}10"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{filter}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{Bicycle (Go Dutch)}\StringTok{\textasciigrave{}} \SpecialCharTok{!=} \StringTok{"0"}\NormalTok{) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{mutate}\NormalTok{(}\FunctionTok{across}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{Bicycle (Baseline)}\StringTok{\textasciigrave{}}\SpecialCharTok{:}\StringTok{\textasciigrave{}}\AttributeTok{Quietness}\StringTok{\textasciigrave{}}\NormalTok{, as.numeric)) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{slice\_max}\NormalTok{(}\StringTok{\textasciigrave{}}\AttributeTok{Bicycle (Go Dutch)}\StringTok{\textasciigrave{}}\NormalTok{, }\AttributeTok{n =} \DecValTok{10000}\NormalTok{)}
\NormalTok{m }\OtherTok{=} \FunctionTok{tm\_shape}\NormalTok{(rnet\_fastest\_city\_top) }\SpecialCharTok{+}
  \FunctionTok{tm\_lines}\NormalTok{(}\AttributeTok{col =} \StringTok{"Quietness"}\NormalTok{, }\AttributeTok{palette =} \StringTok{"RdYlGn"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{5}\NormalTok{)}
\FunctionTok{tmap\_save}\NormalTok{(m, }\StringTok{"./images/rnet\_fastest\_city\_top.png"}\NormalTok{, }\AttributeTok{width =} \DecValTok{10}\NormalTok{, }\AttributeTok{height =} \DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{images/rnet_fastest_city_top.png}

}

\caption{\label{fig-city-results}Top 10,000 routes in Dublin by `cycle
friendliness' (quietness) based on the `fastest' route network.}
