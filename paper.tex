...
CRUSE provides estimates of cycling currently and under `snapshot'
scenarios of `cycling futures' to inform investment decisions. The
results are publicly available at \url{https://cruse.bike/}, enabling
planners, engineers, and other stakeholders to make more evidence-based
decisions. CRUSE goes beyond previous work by: modelling networks at
high levels of geographic resolution; simulating multiple trip purposes
(social, shopping and personal utility trips plus recreational and cycle
touring trips), in addition to using official data on trips to work and
education locations; and by providing estimates of `quietness' (a proxy
for cyclability) at the route segment level. Three network types ---
`Fastest', `Balanced' and `Quietest' --- help plan both arterial and
residential cycle networks. Workshops and unstructured interviews of
stakeholders show that the tool is already being used in practice, to
inform urban, inter-urban and rural network development. The approach is
flexible and open, allowing the underlying ideas and code to be used in
other places to generate the evidence needed for effective cycling
