...
In Portugal, a cycling uptake tool to suport decision making was developed based on PCT methods, for the Lisbon metro region (see [biclaR](https://biclar.tmlmobilidade.pt), and again, socioenvirnment impacts were added as a relevant information to support decisions, using the WHO HEAT for Cycling tool[^3] [@felix2023]. biclaR added on the pevious methods by setting a intermodality scenario of combinining bicycle with the available transit modes (using GTFS), for a maximum time travel and distance on bicycle.
