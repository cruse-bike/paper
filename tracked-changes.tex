...
%DIF DEL paper-v1.tex   Wed Mar 27 00:01:16 2024
%DIF ADD paper.tex      Tue Mar 26 23:46:44 2024
...
%DIF <   pdfauthor={},
%DIF <   pdfkeywords={Cycling, Open-source, Road Safety, Active Travel},
%DIF -------
  ...
  pdfkeywords={Cycling, Open-source, Road Safety, Active %DIF > 
Travel, Transport Planning, Collaborative Planning}, %DIF > 
%DIF -------
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
...
%DIFDELCMD < %%%
\DIFdel{Legislation mandates improved road safety and decarbonisation of
...
}\DIFdelend \DIFaddbegin \cortext[cor1]{Corresponding author}
        \DIFaddend 

\DIFdelbegin \DIFdel{This paper describes the
design, features and potential use of the }\DIFdelend \DIFaddbegin \begin{abstract}
\DIFadd{This paper describes an approach for developing strategic cycle network
planning tools. Based on our experience developing and deploying the
}\DIFaddend Cycle Route Uptake and Scenario Estimation (CRUSE) Tool for Ireland\DIFdelbegin \DIFdel{was
...
at https://cruse.bike/ enable }\DIFaddend planners, engineers, and other
stakeholders to make more evidence-based decisions. CRUSE goes beyond
previous work \DIFdelbegin \DIFdel{in several important ways. It models }\DIFdelend \DIFaddbegin \DIFadd{by: modeling }\DIFaddend networks at high \DIFdelbegin \DIFdel{levels of geographic resolution, simulates a wide range of trips
...
Travel \DIFaddbegin \sep \DIFadd{Transport Planning }\sep 
    \DIFadd{Collaborative Planning
...
\DIFdelbegin \DIFdel{High energy transport systems are a major contributor to climate change}\DIFdelend \DIFaddbegin \subsection{\DIFadd{Background}}\label{background}

\DIFadd{Transport systems dominated by heavy and powerful private cars are
inefficient, dangerous, and unhealthy. Cars are a major and growing
source of greenhouse gas (GHG) emissions \mbox{%DIFAUXCMD
\citep{winkler2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend , a leading
cause of premature death and injury due to road traffic collisions
\DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{globals2018}}\hskip0pt%DIFAUXCMD
}\DIFaddend , and a cause of disease due to \DIFdelbegin \DIFdel{airborne and noise pollutants.
...
save lives. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{physical inactivity
and air, noise and microplastic pollution \mbox{%DIFAUXCMD
\citep{mattsson2023}
}\hskip0pt%DIFAUXCMD
\mbox{%DIFAUXCMD
\citep{welch2023} }\hskip0pt%DIFAUXCMD
\mbox{%DIFAUXCMD
\citep{cavallaro2024}}\hskip0pt%DIFAUXCMD
. }\DIFaddend Transport is responsible for
23\% of \DIFdelbegin \DIFdel{global greenhouse gas (GHG )
}\DIFdelend \DIFaddbegin \DIFadd{GHG }\DIFaddend emissions, 70\% of which are from road transport\DIFdelbegin \DIFdel{. Nearly half of these
}\DIFdelend \DIFaddbegin \DIFadd{, with
passenger cars accounting for nearly half of transport }\DIFaddend emissions (around
10\% of global emissions) \DIFdelbegin \DIFdel{are from passenger cars
}\DIFdelend \citep{jaramillo2022}. The transport system
...
\DIFdelbegin \DIFdel{in Ireland and other countries }\DIFdelend --- outperform electric cars, which are too heavy and expensive\DIFaddbegin \DIFadd{, for the
majority of trips}\DIFaddend . Over-reliance on electric cars could slow the
...
\citep{tait2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
identified and \DIFdelbegin \DIFdel{improved. A proactive approach to the development of
}\DIFdelend \DIFaddbegin \DIFadd{`cycle proofed'. Cycling in Ireland represents only 3\%
...
\citep{brick2018}}\hskip0pt%DIFAUXCMD
, and the need to improve road safety
drives the development of national }\DIFaddend cycling policy and infrastructure
...
\citep{sustaina2019}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend 
...
distances). \DIFdelbegin \DIFdel{7 in 10 }\DIFdelend \DIFaddbegin \DIFadd{Seven in ten }\DIFaddend trips in Ireland are by car \citep{national}.
...
conversations around cycle network planningbetween all stakeholders in
the transport planning process}\DIFdelend \DIFaddbegin \DIFadd{, but also be useful for local network planning}\DIFaddend .

\DIFdelbegin \DIFdel{TII also requires evidence to support monitoring of growth in cycling, upgrades of existing cycling infrastructure, business case development
and appraisal, and exposure information to estimate crash rates. Exposure information is a key part of the the European RISM Directive,
...
point', providing key information for and supporting ambitious plans in many aspects the planning system, from network design to post-build
monitoring \mbox{%DIFAUXCMD
\citep{lovelace2020}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \subsection{\DIFadd{Aim and content}}\label{aim-and-content}

\DIFadd{This paper describes an approach to strategic cycle network planning
...
are publicly available at }\href{https://cruse.bike/}{\DIFadd{cruse.bike}}\DIFaddend .
...
%DIFDELCMD < \includegraphics{images/paste-1.png}
%DIFDELCMD < 

%DIFDELCMD < \includegraphics{images/paste-3.png}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-dublin}%%%
\DIFdelFL{Illustration of the density of route networks
...
%DIFDELCMD < %%%
\DIFdel{This paper aims to describe the design, features and potentialuse of
the CRUSE Tool}\DIFdelend \DIFaddbegin \DIFadd{In Section~\ref{sec-tools}, we review existing current tools for
estimating cycling potential}\DIFaddend . In Section~\ref{sec-methods}, we outline
...
Section~\ref{sec-discussion}, we discuss \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{limitations and possible future
improvements to the approach, and the }\DIFaddend implications of the results for
...
\section{\DIFdelbegin \DIFdel{Methods and data}\DIFdelend \DIFaddbegin \DIFadd{Tools for estimating cycling potential}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{sec-methods}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec-tools}
\DIFaddend 

\DIFdelbegin \DIFdel{The methods used to generate the evidence presented in the CRUSE Tool
build on the Propensity to Cycle Tool (PCT}\DIFdelend \DIFaddbegin \DIFadd{Tools for estimating geographical distribution of cycling potential have
...
support systems (PSS}\DIFaddend ) , which \DIFdelbegin \DIFdel{was originally funded
by the UK's Department for Transport and
developed by a multi-university
team.
An important feature of
the PCT is that it is open source and
publicly available (at }\href{https://www.pct.bike/}{\DIFdel{www.pct.bike}}%DIFAUXCMD
\DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{are designed to help planners and
decision-makers to make more informed decisions \mbox{%DIFAUXCMD
\citep{geertman2009}}\hskip0pt%DIFAUXCMD
.
...
\citep{bencekri2024}}\hskip0pt%DIFAUXCMD
, public transport
\mbox{%DIFAUXCMD
\citep{barmentlo2012} }\hskip0pt%DIFAUXCMD
and cycling \mbox{%DIFAUXCMD
\citep{bencekri2023}}\hskip0pt%DIFAUXCMD
. While such prior
...
\citep{geertman2002}}\hskip0pt%DIFAUXCMD
. We
...
\mbox{%DIFAUXCMD
\citep{lovelace2017}}\hskip0pt%DIFAUXCMD
...
2007 \mbox{%DIFAUXCMD
\citep{parkin_estimation_2007}}\hskip0pt%DIFAUXCMD
...
\citep{transport_for_london_analysis_2010}}\hskip0pt%DIFAUXCMD
.
}
...
\mbox{%DIFAUXCMD
\citep{larsen_build_2013}}\hskip0pt%DIFAUXCMD
...
\citep{phillips_development_2017}}\hskip0pt%DIFAUXCMD
. The CPT is
...
\citep{silva_gross_2021}}\hskip0pt%DIFAUXCMD
. The GPC uses two
...
\citep{lovelace2023}}\hskip0pt%DIFAUXCMD
. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The first version of the PCT was based on }\DIFdelend \DIFaddbegin \DIFadd{10. They are then combined into an
...
\mbox{%DIFAUXCMD
\citep{silva_revealing_2022}}\hskip0pt%DIFAUXCMD
.
}
...
\citep{lovelace2016}}\hskip0pt%DIFAUXCMD
. It was
launched in April }\DIFdelend \DIFaddbegin \DIFadd{for
case study cities \mbox{%DIFAUXCMD
\citep{lovelace_mapping_2016}}\hskip0pt%DIFAUXCMD
. The approach was
...
\citep{cycling2017}}\hskip0pt%DIFAUXCMD
...
\DIFaddbegin \DIFadd{data with }\DIFaddend high levels of geographic resolution are \DIFdelbegin \DIFdel{needed in the data used to estimate
cycling }\DIFdelend \DIFaddbegin \DIFadd{necessary to estimate
cyclical }\DIFaddend potential. However, datasets on travel patterns are often only
...
for OD data disaggregation called `jittering'\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{lovelace2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend . The method works by first
...
\includegraphics{paste-8.png} \includegraphics{paste-9.png}
...
However, travel to school and work \DIFdelbegin \DIFdel{only constitute around only 30\%}\DIFdelend \DIFaddbegin \DIFadd{account for less than half (39\%) }\DIFaddend of
all trips in Ireland\DIFdelbegin \DIFdel{. To overcome }\DIFdelend \DIFaddbegin \DIFadd{, excluding `returning home' trips, according to the
2022 National Household Travel Survey (NHTS) \mbox{%DIFAUXCMD
\citep{national2022}}\hskip0pt%DIFAUXCMD
. This
...
release of the same report \mbox{%DIFAUXCMD
\citep{national2017}}\hskip0pt%DIFAUXCMD
. To tackle }\DIFaddend this issue,
...
and multi-stage trips\DIFdelbegin \DIFdel{(not yet implemented)}\DIFdelend . An overview of the trip purposes used in CRUSE is
presented in \DIFdelbegin \DIFdel{Figure~\ref{fig-trip-purposes}}\DIFdelend \DIFaddbegin \DIFadd{Table~\ref{tbl-trip-purposes}}\DIFaddend .
...
%DIFDELCMD < \includegraphics{images/paste-4.png}
%DIFDELCMD < 

%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begingroup\fontsize{8}{10}\selectfont
\DIFaddendFL 

\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-trip-purposes}%%%
\DIFdelFL{Trip purposes used in the CRUSE Tool.}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \begin{longtable}[t]{>{\raggedright\arraybackslash}p{5em}>{\raggedright\arraybackslash}p{12em}>{\raggedright\arraybackslash}p{15em}>{\raggedright\arraybackslash}p{15em}}
\DIFaddendFL 

\DIFdelbeginFL %DIFDELCMD < \end{figure}%%%
%DIF < 
\DIFdelend \DIFaddbegin \caption{\label{tbl-trip-purposes}\DIFadd{Trip purposes used in the CRUSE Tool.}}
\DIFaddend 

\DIFaddbegin \tabularnewline

\toprule
\DIFadd{Trip purpose }& \DIFadd{Description }& \DIFadd{Primary source(s) }& \DIFadd{Confidence}\\
\midrule
...
\DIFadd{Primary Education }& \DIFadd{Primary school trips }& \DIFadd{Census 2016 POWSCAR Data, Schools database }& \DIFadd{High confidence - data both origins and destinations for trip purposes}\\
...
\DIFadd{Tertiary Education }& \DIFadd{Tertiary education trips }& \DIFadd{Census 2016 POWSCAR Data - Geodirectory categories }& \DIFadd{High confidence - data both origins and destinations for trip purposes}\\
...
\addlinespace
\DIFadd{Shopping }& \DIFadd{NHTS classificationfrom supermarkets and shops }& \DIFadd{Trip rates from NHTS, Geodirectory businesses classified according to relevant NACE codes }& \DIFadd{Medium confidence}\\
...
\DIFadd{Tourism / recreational }& \DIFadd{Trips by non-resident visitors to / from visitor attractions }& \DIFadd{Geodirectory listings for accommodation (origins) and destinations (destinations), Failte Ireland accommodation database, Failte Ireland attraction database }& \DIFadd{Low confidence - lack of data regarding tourist trip rates}\\
\bottomrule
...
\DIFadd{Following feedback from stakeholders, we added another, `non-everyday'
trip purpose in an extension phase: cycle tourism. As outlined in the
CRUSE Extension report }{[}\DIFadd{reference and link to be added on
publication}{]}\DIFadd{, this involved developing an inter-county spatial
...
\DIFaddend \subsection{Routing}\label{routing}
...
for cycle planning and advocacy \DIFdelbegin \DIFdel{. Although based in Cambridge, UK,
CycleStreets provide routing services `by cyclists, for cyclists'
}\DIFdelend internationally, including in Ireland.
...
\DIFdelbegin \DIFdel{We calculated three network }\DIFdelend \DIFaddbegin \DIFadd{The CycleStreets routing engine accounts for hilliness and traffic
...
\DIFadd{We computed three route }\DIFaddend types for each disaggregated (`jittered') OD
pair: `Fastest', `Balanced' and `Quietest'\DIFaddbegin \DIFadd{, based on the observation
...
\mbox{%DIFAUXCMD
\citep{desjardins2022}}\hskip0pt%DIFAUXCMD
}\DIFaddend . As outlined on the CycleStreets'
\href{https://www.cyclestreets.net/help/journey/howitworks/}{website},
the \DIFdelbegin \DIFdel{`Fastest' }\DIFdelend \DIFaddbegin \DIFadd{fastest }\DIFaddend routes minimize journey time, accounting for traffic lights
...
a \DIFdelbegin \DIFdel{compromize }\DIFdelend \DIFaddbegin \DIFadd{compromise }\DIFaddend between the two, \DIFdelbegin \DIFdel{minimising
}\DIFdelend \DIFaddbegin \DIFadd{minimizing }\DIFaddend journey time while avoiding the
...
Figure~\ref{fig-route-types}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{These map }\DIFdelend \DIFaddbegin \DIFadd{These different network maps }\DIFaddend are available
on the `Route types' page for each county (see
\href{https://cruse.bike/kildare/route-types}{cruse.bike/kildare/route-types}
\DIFaddbegin \DIFadd{for Kildare, for example}\DIFaddend ). In addition to showing the estimated routes
...
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.60417in,height=\textheight]{images/naas_quietest_godutch.png}%%%
...
%
\begin{minipage}{0.50\linewidth}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.60417in,height=\textheight]{images/naas_fastest_godutch.png}%%%
...
\newline
\begin{minipage}{0.50\linewidth}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.60417in,height=\textheight]{images/newbridge_quietest_godutch.png}%%%
...
%
\begin{minipage}{0.50\linewidth}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.60417in,height=\textheight]{images/newbridge_fastest_godutch.png}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=2.60417in,height=\textheight]{newbridge_fastest_godutch.png}\DIFaddendFL \end{minipage}%

\caption{\label{fig-route-types}Illustration of the \DIFdelbeginFL \DIFdelFL{Quietest }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{quietest }\DIFaddendFL (left) and
\DIFdelbeginFL \DIFdelFL{Fastest }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{fastest }\DIFaddendFL (right) route networks for Naas (top) and Newbridge (bottom) \DIFaddbeginFL \DIFaddFL{in
County Kildare}\DIFaddendFL .}
...
%DIFDELCMD < \includegraphics{images/scenarios_kildare.png}
%DIFDELCMD < 
...
The Climate Action Plan scenario \DIFdelbegin \DIFdel{is loosely based on the }\DIFdelend \DIFaddbegin \DIFadd{models the transport emissions
reductions targeted in the }\DIFaddend Irish Government's
\href{https://www.gov.ie/en/publication/6223e-climate-action-plan-2021/}{Climate
Action Plan 2021}, which \DIFdelbegin \DIFdel{contains policies for action to achieve }\DIFdelend \DIFaddbegin \DIFadd{aims for }\DIFaddend a 51\% \DIFdelbegin \DIFdel{reduction }\DIFdelend \DIFaddbegin \DIFadd{cut }\DIFaddend in overall GHG emissions by
2030 \DIFdelbegin \DIFdel{, on a path towards }\DIFdelend \DIFaddbegin \DIFadd{as part of the pathway to }\DIFaddend net-zero \DIFaddbegin \DIFadd{emissions }\DIFaddend by 2050. For
...
To model this decrease in car \DIFdelbegin \DIFdel{driver km }\DIFdelend \DIFaddbegin \DIFadd{km driven}\DIFaddend , cycling uptake increases in
...
uses the same model \DIFdelbegin \DIFdel{that was used }\DIFdelend \DIFaddbegin \DIFadd{as the }\DIFaddend PCT, allowing trips to shift from any other
...
The CRUSE web application is statically hosted, meaning \DIFdelbegin \DIFdel{that it can be
accessed without the need for }\DIFdelend \DIFaddbegin \DIFadd{it does not
require }\DIFaddend a server running software such as the R package `shiny' or
Python packages such as `streamlit' or `flask' in the background
\citep{wickham2021}. \DIFaddbegin \DIFadd{This reduces hosting costs and the maintenance
...
format \mbox{%DIFAUXCMD
\citep{gonçalves2023}}\hskip0pt%DIFAUXCMD
. These open source technologies were used
to generate default view in the landing page: a map of Ireland with
county level results for the \% trips that could be made by cycling
under the `Go Dutch' scenario, highlighting the intended use case for
strategic cycle network planning and level of ambition, as illustrated
in Figure~\ref{fig-landing}.
}
...
\caption{\label{fig-landing}\DIFaddFL{Illustration of the landing page of the
...
`zoomed in' view shows the national network view for the `Go Dutch'
scenario for the Limerick area (bottom).}}

\end{figure}%DIF > 

\DIFadd{The key results, route network results, are available within an instant
...
\DIFadd{Other improvements to the user interface in the national map on the
...
\DIFaddend Typical intended user stories are illustrated in
...
%DIFDELCMD < \includegraphics{images/mermaid-figure-1.png}
%DIFDELCMD < 
...
\includegraphics{fig-user-stories-1.pdf}
...
\DIFaddend To illustrate the results in urban areas, we \DIFdelbegin \DIFdel{took the top 4 }\DIFdelend \DIFaddbegin \DIFadd{explored route networks in
the three largest }\DIFaddend cities in Ireland\DIFdelbegin \DIFdel{by population: DublinCity}\DIFdelend \DIFaddbegin \DIFadd{: Dublin}\DIFaddend , Cork, \DIFdelbegin \DIFdel{Limerick and Galway (with city
populations of 0.6, 0.2, 0.1 }\DIFdelend and \DIFdelbegin \DIFdel{0.1 million respectively).
In
}\DIFdelend \DIFaddbegin \DIFadd{Limerick.
}\DIFaddend Figure~\ref{fig-city-results} \DIFdelbegin \DIFdel{we present the cycle route }\DIFdelend \DIFaddbegin \DIFadd{shows the `fastest' and `quietest' cycle
}\DIFaddend networks within a \DIFdelbegin \DIFdel{3km radius circle }\DIFdelend \DIFaddbegin \DIFadd{6km wide square }\DIFaddend centred on each of these cities.
...
%DIFDELCMD < \includegraphics{images/rnet_types.png}
%DIFDELCMD < 
...
\includegraphics{rnet_types.png}

}
\DIFaddendFL 

\caption{\label{fig-city-results}Fastest \DIFdelbeginFL \DIFdelFL{, balanced }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(left) }\DIFaddendFL and quietest \DIFaddbeginFL \DIFaddFL{(right)
}\DIFaddendFL cycle networks \DIFdelbeginFL \DIFdelFL{within 3km circular areas containing the four largest cities }\DIFdelendFL in \DIFdelbeginFL \DIFdelFL{Ireland}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6 km squares centered on central Dublin (top panels)
Cork (middle) and Limerick (bottom)}\DIFaddendFL . The networks are coloured by cycle
...
Aside from the \DIFdelbegin \DIFdel{widely }\DIFdelend varying sizes and route \DIFdelbegin \DIFdel{networks of }\DIFdelend \DIFaddbegin \DIFadd{network shapes associated with
}\DIFaddend each city, \DIFdelbegin \DIFdel{with Dublin having by far the densest network, }\DIFdelend it is clear from the visualisation of cycling friendliness
that \DIFdelbegin \DIFdel{there are many gaps in the networks. Dublin has had received most investment in cycling
infrastructureand this is apparent from the }\DIFdelend \DIFaddbegin \DIFadd{none of the cities can be considered to have a `complete' cycle
network. Of the three, Dublin has clearly had most investment cycle
infrastructure, as shown by the relatively }\DIFaddend high proportion of the route
network that is green \DIFdelbegin \DIFdel{, even in the `Fastest' route network representing routes that prioritize speed and directness over comfort
...
cycling infrastructure is effective in increasing cycling uptake. }\DIFdelend \DIFaddbegin \DIFadd{in both network types.
}\DIFaddend 

\DIFdelbegin \DIFdel{Although Dublin City has made progress, there are }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{fig-city-results} highlights the value of generating
...
less confident cycling }\emph{\DIFadd{cannot}} \DIFadd{avoid mixing with motor traffic for
...
\DIFadd{There are }\DIFaddend still many parts of the \DIFdelbegin \DIFdel{Fastest }\DIFdelend \DIFaddbegin \DIFadd{fastest }\DIFaddend route network, and even some
parts of the \DIFdelbegin \DIFdel{Balanced and
Quietest }\DIFdelend \DIFaddbegin \DIFadd{quietest }\DIFaddend route networks, that are not cycle friendly and
which have high cycling potential\DIFaddbegin \DIFadd{, even in Dublin}\DIFaddend . According to a recent
...
pedestrians'' \citep{walking2021}. \DIFdelbegin \DIFdel{The results for the Dublin area could help prioritise investment in
cycling infrastructure in the city}\DIFdelend \DIFaddbegin \DIFadd{Assuming that these findings apply in
...
\subsection{\DIFadd{Stakeholder engagement and feedback}}\label{sec-qualitative}

\DIFadd{Throughout the project, we undertook a series of workshops and
...
\DIFadd{Practitioners in Kildare County Council found that the evidence
generated }\emph{\DIFadd{was already valuable}} \DIFadd{for the development of new cycle
...
assist in the development of new cycling projects}\DIFaddend .

\DIFaddbegin \DIFadd{During the workshop with practitioners in Limerick, the estimates of
...
\DIFadd{There were two suggestions from Declan which have not yet been
...
with potential assigned to \DIFdelbegin \DIFdel{`fastest', `balanced ' and `quietest ' }\DIFdelend \DIFaddbegin \DIFadd{fastest, balanced and quietest }\DIFaddend route
...
\DIFaddend While the CRUSE \DIFdelbegin \DIFdel{Tool }\DIFdelend \DIFaddbegin \DIFadd{approach presented in this paper }\DIFaddend is not the first
...
\tightlist
\item
  \DIFaddbegin \DIFadd{Compared with the Propensity to Cycle Tool (PCT), which provides
  either travel to work \mbox{%DIFAUXCMD
\citep{lovelace2017} }\hskip0pt%DIFAUXCMD
or to school cycling
  networks \mbox{%DIFAUXCMD
\citep{goodman2019}}\hskip0pt%DIFAUXCMD
, the approach presented here captures a
  ...
  \DIFaddend The results are \emph{open access} meaning that any stakeholder in the
...
\item
  \DIFdelbegin \DIFdel{By covering a wide range of trip purposes --- not just travel to work
  \mbox{%DIFAUXCMD
\citep{lovelace2017, heinen2010} }\hskip0pt%DIFAUXCMD
and travel school \mbox{%DIFAUXCMD
\citep{goodman2019}
  }\hskip0pt%DIFAUXCMD
as covered in previously published research on open access national
  ...
  proportion of cycling potential including key
  rural trips which are
  often under-represented in models of active travel}\DIFdelend \DIFaddbegin \DIFadd{The tool has a user interface that allows people to see the key
  results quickly, without having to click on regional result pages, as
  is the case with the BiclaR and PCT projects}\DIFaddend .
\end{itemize}

As the benefits of strategic planning for cycling become more apparent
\citep{scappini2022}\DIFaddbegin \DIFadd{, }\DIFaddend we expect the demand for \DIFdelbegin \DIFdel{tools such as CRUSE }\DIFdelend \DIFaddbegin \DIFadd{the approach outlined in
this paper, and resulting tools, }\DIFaddend to grow. \DIFaddbegin \DIFadd{However, the approach is not
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Limitations and future
research}}\label{limitations-and-future-research}
\DIFaddend 

\DIFdelbegin \DIFdel{The CRUSE Tool is an open access web application for strategic cycle
...
evidence base, providing valuable insights to
planners and other
stakeholders in the transport planning process, at national and local
levels. Because the results are available at the route segment level, }\DIFdelend \DIFaddbegin \DIFadd{The approach has a number of limitations that should be understood by
...
}{[}\DIFadd{reference to be added on publication}{]}\DIFadd{, we also undertook validation
...
network expands, allowing refinements to }\DIFaddend the \DIFdelbegin \DIFdel{tool can be used to identify `weak links'
in the cycle network
\mbox{%DIFAUXCMD
\citep{vybornova2022}}\hskip0pt%DIFAUXCMD
, and to prioritize investment in cycling
infrastructure to maximize health, equality, and other benefits
\mbox{%DIFAUXCMD
\citep{mahfouz, woodcock2021}}\hskip0pt%DIFAUXCMD
. Furthermore, the provision of the
evidence in a free and publicly available website, hosted at
}\href{https://cruse.bike}{\DIFdel{cruse.bike}}%DIFAUXCMD
\DIFdel{, means that it can be used by
anyone, encouraging wider participation and more evidence-based debate
about transport planning. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{A key feature of the project methodologically is its calculation of current and future potentialnot only for travel to work and travel to
school, but also for other trip purposes, including recreational trips.
This required the development of spatial interaction models and estimation of the relative attractiveness of different destinations for
different trip purposes, an area of active research where new
developments could be incorporated into the
tool in future
\mbox{%DIFAUXCMD
\citep{hasova2022}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{approach (espcially the
...
application}\DIFaddend .

\DIFdelbegin \DIFdel{The CRUSE Tool and the underlying research and methods are not without
limitations, suggesting future areas of research, datacollection needs,
and policy application. The route network level results }\DIFdelend \DIFaddbegin \DIFadd{A limitation associated with the approach is its reliance on
...
However, there are many other potential errors in the OSM data that }\DIFaddend have
not been \DIFdelbegin \DIFdel{validated to the
extent we had planned at the outset of the project. We
compared route network level results with `ground truth' data from cycle
counter datasets across Dublin to test different network types and }\DIFdelend \DIFaddbegin \DIFadd{corrected. Future work could mitigate this by integrating the
...
valuable if not essential }\DIFaddend to ensure that \DIFdelbegin \DIFdel{additional trip types added to work and educational trips
increased the quality of
fit under the
baseline scenario. However, }\DIFdelend the \DIFdelbegin \DIFdel{size of the counter network was insufficient to provide an opportunity
...
outputs }\DIFdelend \DIFaddbegin \DIFadd{tool is used effectively.
...
\DIFadd{Another limitation of the approach is that the results are `static',
...
key part }\DIFaddend of the the \DIFdelbegin \DIFdel{CRUSE Tool}\DIFdelend \DIFaddbegin \DIFadd{European RISM Directive, for reporting collision
...
limitation, although would take substantial resources}\DIFaddend .
...
\DIFaddbegin \section{\DIFadd{Conclusions}}\label{sec-conclusions}

\DIFaddend The CRUSE Tool is \DIFaddbegin \DIFadd{an open access web application for strategic cycle
network planning and prioritization of road safety interventions across
Ireland. Building on previous work, it provides a nationally consistent
...
. Furthermore, the provision of the
evidence in a free and publicly available website, hosted at
}\href{https://cruse.bike}{\DIFadd{cruse.bike}}\DIFadd{, means that it can be used by
anyone, encouraging wider participation and more evidence-based debate
about transport planning.
}

\DIFadd{A key feature of the project methodologically is its calculation of
...
\DIFadd{As outlined in Section~\ref{sec-qualitative}, the tool is }\DIFaddend already used
...
underlying approach \DIFdelbegin \DIFdel{, and the publicly
available evidence provided in the CRUSE Tool itself, provide }\DIFdelend \DIFaddbegin \DIFadd{provides }\DIFaddend a basis for future \DIFdelbegin \DIFdel{reproducible research , open source software development into
strategic cycle network planning tools in Ireland and
other countries. Active travel represents a win-win-win for physical activity,
environment, and local economic opportunities. }\DIFdelend \DIFaddbegin \DIFadd{research and
development. }\DIFaddend In combination with broader sustainable mobility measures
and policies to reduce motor traffic speeds and volumes, \DIFdelbegin \DIFdel{tools such as CRUSE }\DIFdelend \DIFaddbegin \DIFadd{evidence
generated by the approach outlined in this paper }\DIFaddend can support the fast
