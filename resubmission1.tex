...
%DIF DEL paper-v1.tex   Fri Mar 22 15:37:12 2024
%DIF ADD paper.tex      Fri Mar 22 15:38:12 2024
...
%DIF -------
  pdftitle={Cycle Route Uptake and Scenario Estimation (CRUSE): an open and nationally scalable approach for strategic cycle network planning tool development}, %DIF > 
%DIF -------
...
\begin{frontmatter}
\title{\DIFaddbegin \DIFadd{Cycle Route Uptake and Scenario Estimation (}\DIFaddend CRUSE\DIFdelbegin \DIFdel{to Safe Cycling in Ireland }\DIFdelend \DIFaddbegin \DIFadd{): an open and
nationally scalable approach for strategic cycle network planning tool
development }\DIFaddend \\\DIFdelbegin %DIFDELCMD < \large{An Open Source Tool to
%DIFDELCMD < Support Active Travel} %%%
\DIFdelend \DIFaddbegin \large{A case study of Ireland} \DIFaddend }
...
levels could greatly assist in achieving this goal. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This paper describes the design, features and potential use of }\DIFdelend \DIFaddbegin \DIFadd{This paper describes an approach to the development of strategic cycle
network planning tools. Based on our experience developing and deploying
}\DIFaddend the Cycle Route Uptake and Scenario Estimation (CRUSE) Tool for Ireland\DIFdelbegin \DIFdel{was
commissioned to support cycle network planning, safety interventions, business cases and project evaluation. The resulting web application is }\DIFdelend \DIFaddbegin \DIFadd{,
...
Infrastructure Ireland and is publicly }\DIFaddend available at
...
previous work \DIFdelbegin \DIFdel{in several important ways. It models }\DIFdelend \DIFaddbegin \DIFadd{by: modelling }\DIFaddend networks at high levels of geographic
resolution\DIFdelbegin \DIFdel{, simulates a wide range of trips
including recreational, }\DIFdelend \DIFaddbegin \DIFadd{; simulating multiple trip purposes (}\DIFaddend social, shopping and
personal \DIFdelbegin \DIFdel{trips}\DIFdelend \DIFaddbegin \DIFadd{utility trips plus recreational and cycle touring trips)}\DIFaddend , in
addition to \DIFaddbegin \DIFadd{using official data on }\DIFaddend trips to work and \DIFdelbegin \DIFdel{primary, secondary and tertiary education
...
route calculation and ``cycle friendliness''
estimates of all links on the network. CRUSE provides }\DIFdelend \DIFaddbegin \DIFadd{education
locations; provision of estimates of `quietness' (a proxy for
cyclability) at the route segment level; and its provision of }\DIFaddend three
network types: \DIFdelbegin \DIFdel{the }\DIFdelend `Fastest'\DIFdelbegin \DIFdel{network highlights routes for directness;
`Quietest' prioritises cycle friendliness, and `}\DIFdelend \DIFaddbegin \DIFadd{, `}\DIFaddend Balanced' \DIFdelbegin \DIFdel{represents a
balance between speed and comfort. Trips are generated based on origin
and destination data from the 2016 Census in
combination with modeled
demand data to estimate current and potential future cycling levels at
area, route, }\DIFdelend and \DIFdelbegin \DIFdel{network levels for each county in Ireland.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For investment in cycling to deliver on the huge potential benefits,
cycling must become safer.
With growth in the ebike market, CRUSE will
help inform }\DIFdelend \DIFaddbegin \DIFadd{`Quietest' to help plan both
...
practice, to inform urban, }\DIFaddend inter-urban and \DIFdelbegin \DIFdel{rural networks }\DIFdelend \DIFaddbegin \DIFadd{rural network development.
The approach and methods for strategic cycle network development tools
outlined in this paper can be applied and built-on in other places }\DIFaddend to
support the \DIFdelbegin \DIFdel{transfer of
...
with sustainable transport policies worldwide}\DIFdelend \DIFaddbegin \DIFadd{generation of evidence needed to support ambitious cycling
...
\end{keyword}
\end{frontmatter}
    \DIFdelbegin %DIFDELCMD < \ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[boxrule=0pt, enhanced, sharp corners, frame hidden, interior hidden, borderline west={3pt}{0pt}{shadecolor}, breakable]}{\end{tcolorbox}}\fi
%DIFDELCMD < %%%
\DIFdelend 
...
identified and improved. \DIFaddbegin \DIFadd{Cycling in Ireland represents only 3\% of total
modal share as of the latest Census (2016), but accounts for 20\% of
serious injuries and 7\% of all fatalities. }\DIFaddend A proactive approach to the
development of cycling policy and infrastructure is taking place in
Ireland. The main frameworks underpinning these efforts are the Climate
Action Plan \citep{climate2022a}, the National Development Plan, and the
National Roads 2040 strategy \citep{national2023}. These policy
frameworks are backed-up by public support for active travel
interventions: while only 3\% of trips in Ireland are made by cycling
according to the latest Census data from 2016 ``83\% of respondents
supported enhanced walking and cycling facilities''
\citep{sustaina2019}.
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec-methods}{%
%DIFDELCMD < \section{Methods and data}\label{sec-methods}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec-disaggregation}{%
%DIFDELCMD < \subsection{Disaggregation of origin-destination
%DIFDELCMD < data}\label{sec-disaggregation}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec-trip-purposes}{%
%DIFDELCMD < \subsection{Additional trip purposes}\label{sec-trip-purposes}}
%DIFDELCMD < %%%
...
%DIFDELCMD < \subsection{Routing}\label{routing}}
%DIFDELCMD < %%%
...
\includegraphics[width=2.60417in,height=\textheight]{images/naas_quietest_godutch.png}\DIFaddendFL \end{minipage}%
%
\DIFdelbeginFL %DIFDELCMD < \begin{minipage}[t]{0.50\linewidth}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{minipage}{0.50\linewidth}
\includegraphics[width=2.60417in,height=\textheight]{images/naas_fastest_godutch.png}\DIFaddendFL \end{minipage}%
\newline
\DIFdelbeginFL %DIFDELCMD < \begin{minipage}[t]{0.50\linewidth}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{minipage}{0.50\linewidth}
\includegraphics[width=2.60417in,height=\textheight]{images/newbridge_quietest_godutch.png}\DIFaddendFL \end{minipage}%
%
\DIFdelbeginFL %DIFDELCMD < \begin{minipage}[t]{0.50\linewidth}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{minipage}{0.50\linewidth}
\includegraphics[width=2.60417in,height=\textheight]{images/newbridge_fastest_godutch.png}\DIFaddendFL \end{minipage}%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec-scenarios}{%
%DIFDELCMD < \subsection{Scenarios of cycling potential}\label{sec-scenarios}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Scenarios of cycling potential}}\label{sec-scenarios}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{baseline}{%
%DIFDELCMD < \subsubsection{Baseline}\label{baseline}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Baseline}}\label{baseline}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{near-market}{%
%DIFDELCMD < \subsubsection{Near Market}\label{near-market}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Near Market}}\label{near-market}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{climate-action-plan}{%
%DIFDELCMD < \subsubsection{Climate Action Plan}\label{climate-action-plan}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Climate Action Plan}}\label{climate-action-plan}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{go-dutch}{%
%DIFDELCMD < \subsubsection{Go Dutch}\label{go-dutch}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{ebike}{%
%DIFDELCMD < \subsubsection{Ebike}\label{ebike}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec-ui}{%
%DIFDELCMD < \subsection{User interface}\label{sec-ui}}
%DIFDELCMD < %%%
...
%DIFDELCMD < \section{Results}\label{sec-results}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec-discussion}{%
%DIFDELCMD < \section{Discussion}\label{sec-discussion}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec-conclusions}{%
%DIFDELCMD < \section{Conclusions}\label{sec-conclusions}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{list-of-abbreviations}{%
%DIFDELCMD < \section{List of abbreviations}\label{list-of-abbreviations}}
%DIFDELCMD < %%%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{declarations}{%
%DIFDELCMD < \section{Declarations}\label{declarations}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section{\DIFadd{Declarations}}\label{declarations}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \hypertarget{availability-of-data-and-material}{%
%DIFDELCMD < \subsection*{Availability of data and
%DIFDELCMD < material}\label{availability-of-data-and-material}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Availability of data and
material}}\label{availability-of-data-and-material}
\DIFaddend \addcontentsline{toc}{subsection}{Availability of data and material}
